    ...
    <section className="sandbox-options" aria-labelledby="sandbox-options-title">
      <div className="sandbox-options-heading">
        <div>
          ...
          <p>Choose whether the host runs once or streams frames through persistent Trace memory.</p>
        </div>
      ...
      <div className="sandbox-options-grid">
        <label className="sandbox-option sandbox-option-wide">
          ...
          <small>
            {settings.runMode === 'once' && 'Evaluate the script once and show its result and console.'}
            {settings.runMode === 'persistent' && 'Re-evaluate the full script each frame in the same memory.'}
            {settings.runMode === 'functions' && 'Register definitions once, pass arguments to setup(), then call tick() per frame.'}
          </small>
        </label>
      ...
      {isLive && (
        <p className="sandbox-options-note">
          Named echoes such as <code>@=position@</code> automatically become labeled time-series lines.
        </p>
      )}
    </section>
  )
