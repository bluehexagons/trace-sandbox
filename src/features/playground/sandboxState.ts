...
import { parseSandboxUrl } from './sandboxUrl';
...
export const normalizeFramesPerSecond = (value: number) =>
  Math.min(
    60,
    Math.max(1, Number.isFinite(value) ? value : defaultSandboxSettings.framesPerSecond),
  );
