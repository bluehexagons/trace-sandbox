...
tick() => {
  ...
  },
  {
    ...
    description:
      'Watch one neighboring comparison per tick and reshuffle the array without restarting the host.',
    concepts: [
      'sorting algorithms',
      'loop invariants',
      'memory-backed output',
    ...
    challenge:
      'Shuffle halfway through a pass, or change the comparison to build a descending sort.',
    code: `fillRandom() => {
  values[i] = 1~100;
  i++ <= size ? >fillRandom() : 0
};
...
tick() => {
  ...
  sorted
    ...
    animation: {
      ...
      description:
        'Gold marks the pair being compared; each complete pass moves large values to the right.',
      framesPerSecond: 15,
      execution: {
        mode: 'live',
    ...
    controls: {
      description:
        'Choose the problem size, then interrupt the running algorithm with a new permutation.',
      autoRun: true,
      items: [
        {
          id: 'size',
        ...
        },
        {
          ...
          label: 'Shuffle array',
          description:
            'Replace every value and restart the sort inside the same memory environment.',
          kind: 'trigger',
  ...
  },
  {
    ...
    description:
      'Estimate π from random points in a square and watch statistical noise settle over time.',
    concepts: [
      'Monte Carlo methods',
      'probability',
      'convergence',
    ...
    challenge:
      'Compare small and large batches, then reset both and see which estimate stabilizes first.',
...
tick() => {
  ...
  frame++
    ...
    animation: {
      ...
      yMax: 4,
      historyLength: 240,
      lines: [{ channel: 'estimate', color: '#a78bfa', label: 'Running estimate' }],
      references: [{ value: 3.141592653589793, color: '#fbbf24', label: 'π reference' }],
    },
    controls: {
