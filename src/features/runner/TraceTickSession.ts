...
export class TraceTickSession {
  ...
  constructor(
    ...
    try {
      ...
      const functionNames = [execution.setupFunction, execution.tickFunction].filter(
        (name): name is string => name !== undefined,
      );
      const invalidFunction = functionNames.find((name) => !/^[a-zA-Z_][\w.]*$/.test(name));
      this.parseError =
        invalidFunction === undefined ? null : `Invalid live function name "${invalidFunction}".`;
      this.setupArgumentNames = this.args.map((_, index) => `__sandbox_arg_${index + 1}`);
      this.setupCall =
        execution.setupFunction === undefined || invalidFunction !== undefined
          ? null
          : Trace.parse(`${execution.setupFunction}(${this.setupArgumentNames.join(',')})`);
      this.tickCall =
        execution.tickFunction === undefined || invalidFunction !== undefined
          ? null
  ...
  tick(): PlaygroundResult {
    ...
    if (this.execution.tickFunction !== undefined && !this.prepared) {
      ...
      if (this.execution.setupFunction !== undefined) {
        if (!this.memory.functions.has(this.execution.setupFunction) || this.setupCall === null) {
          return runError(
            'error',
            `Live setup function "${this.execution.setupFunction}" is not defined.`,
          );
        }
    ...
    if (
      ...
      return runError(
        'error',
        `Live tick function "${this.execution.tickFunction}" is not defined.`,
      );
    }
